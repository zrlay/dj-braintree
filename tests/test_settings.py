...
class TestSubscriberModelRetrievalMethod(TestCase):
    ...
    def test_bad_model_name(self):
        self.assertRaisesMessage(ImproperlyConfigured, "DJBRAINTREE_PAYER_MODEL must be of the form 'app_label.model_name'.", get_payer_model)
    ...
    def test_unknown_model(self):
        self.assertRaisesMessage(ImproperlyConfigured, "DJBRAINTREE_PAYER_MODEL refers to model 'testapp.UnknownModel' that has not been installed.", get_payer_model)
    ...
    def test_no_email_model(self):
        self.assertRaisesMessage(ImproperlyConfigured, "DJBRAINTREE_PAYER_MODEL must have an email attribute.", get_payer_model)

    @override_settings(DJBRAINTREE_PAYER_MODEL='testapp.Organization')
    def test_no_callback(self):
        self.assertRaisesMessage(ImproperlyConfigured, "DJBRAINTREE_PAYER_MODEL_REQUEST_CALLBACK must be implemented if a DJBRAINTREE_PAYER_MODEL is defined.", get_payer_model)

    @override_settings(DJBRAINTREE_PAYER_MODEL='testapp.Organization', DJBRAINTREE_PAYER_MODEL_REQUEST_CALLBACK=5)
    def test_bad_callback(self):
        self.assertRaisesMessage(ImproperlyConfigured, "DJBRAINTREE_PAYER_MODEL_REQUEST_CALLBACK must be callable.", get_payer_model)
